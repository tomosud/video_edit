...
- Title字幕を選択すると、横・縦モニター上の文字ブロックをそれぞれドラッグして位置変更できる。
- Title位置は字幕バー単位で保存し、横16:9と縦9:16は独立する。
- 各モニターの `Reset Title Position` は、その向きだけ中央へ戻す。
- 旧データのように `titlePosition` がないTitleは中央 `{ x: 0.5, y: 0.5 }` として表示する。
...
## Title位置編集 (2026-07-16)

- 選択中のTitle字幕を横・縦モニター上で直接ドラッグできるようにした。
- 位置は各captionの任意 `titlePosition.horizontal / vertical` に正規化座標で保存する。
- プレビューとMP4書き出しで同じ位置を使用する。
- 位置情報のない既存データは従来どおり中央表示する。
