...
```
┌──────────────────────────────────────────────────────────┐
│  [ファイルを開く]  [VTT追加]  [BGM追加]  [書き出し]       │  ← ヘッダー
├─────────────────────────────┬────────────────────────────┤
│  ソース動画プレイヤー        │  出力プレビュー（9:16）    │
│  ┌───────────────────────┐  │  ┌──────────────────────┐  │
│  │  <video>              │  │  │  縦型 Canvas         │  │
│  │                       │  │  │  字幕オーバーレイ    │  │
│  └───────────────────────┘  │  └──────────────────────┘  │
│  字幕テキスト（字幕行クリック│  テキスト編集パネル         │
│  でシークできる）            │  位置・サイズ・色           │
├─────────────────────────────┴────────────────────────────┤
│  サムネイルストリップ（横スクロール、ズーム可）            │  ← ソース
│  ▐████░░░░[IN]░░░░░░░░░░░░░[OUT]░░░░░░▌ …              │
│  [字幕行]▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓▓                  │
├──────────────────────────────────────────────────────────┤
│  出力クリップ列（ドラッグ並び替え可）                      │  ← 出力
│  [Clip 1][Clip 2][Clip 3] + [追加]                       │
│  ▐ BGM トラック ░░░░░░░░░░░░░░░░░░░░░░ ▌              │
└──────────────────────────────────────────────────────────┘
```
...
- **固定3行レイアウト**: ヘッダー / 上段ビュー / 中段ストリップ / 下段クリップ列を CSS Grid の行で割り付け、縦スクロールなしで全機能が収まる高さ配分にする（例: ヘッダー 56px / 上段 1fr / 中段 200px / 下段 180px）。
...
│   └── style.css
├── js/
│   ├── app.js            # エントリポイント、イベント配線
│   ├── fileOpen.js       # File System Access API ラッパー
│   ├── thumbnails.js     # WebCodecs サムネイルストリップ
│   ├── trimTimeline.js   # IN/OUT ポイント UI
│   ├── clipList.js       # 出力クリップ管理・並び替え
│   ├── cropPreview.js    # 9:16 Canvas プレビュー・パン
│   ├── subtitles.js      # VTT パース・表示・字幕行 UI
│   ├── whisper.js        # Whisper WASM ラッパー
│   ├── textOverlay.js    # テキスト／字幕オーバーレイ編集
│   ├── bgm.js            # BGM AudioBuffer 管理・音量
│   ├── export.js         # FFmpeg.wasm エンコード・ダウンロード
│   ├── projectStore.js   # プロジェクトフォルダ I/O（project.json 読み書き）
│   ├── db.js             # IndexedDB ラッパー（自動保存・ハンドル・モデルキャッシュ）
│   └── history.js        # Undo/Redo スタック（編集状態のスナップショット）
├── lib/
